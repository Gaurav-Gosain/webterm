...
flowchart TB
  ...
  subgraph Core["webterm"]
    ...
    Sync[SyncOutputWatchdog<br/>mode 2026 repaint ceiling]
...
| `forcedRepaints` | `number` | Repaints the mode 2026 watchdog has had to force. Zero for every application that ends its synchronized updates promptly |
...
| `syncOutput` | `timeoutMs` (`150`), the ceiling on a repaint withheld by DEC mode 2026 |
...
DEC private mode 2026 lets an application bracket a screen update so the
terminal does not show it half drawn. xterm.js honours it, and under sustained
output it honours it to death: the terminal stops repainting entirely and stays
stopped for as long as the output lasts.

The mechanism is a second mode check. `RenderService.refreshRows` buffers the
damaged rows while the mode is set, which is correct. `RenderService._renderRows`
re-checks the mode inside the animation frame and declines to draw, which is
correct only if the mode is still set for the reason the frame was scheduled.
An end-of-update schedules a frame, parsing continues in the same task, the next
begin-of-update sets the mode again microseconds later, and the frame that lands
up to 16 ms afterwards finds the mode set and draws nothing. The debouncer has
already cleared its pending range, so the refresh is dropped rather than
deferred. xterm carries a 1000 ms safety timeout for this, but it is armed when
rows are buffered and cleared on every end-of-update, so the event it protects
against missing is the event that disarms it, and under sustained updates it
never fires.

Measured against a sip session at 225x55 on an RTX 3070, replaying a btop
capture whose bytes are 100% bracketed in mode 2026, five repetitions each:

| | renders/s | distinct framebuffers | MiB/s |
| --- | --- | --- | --- |
| xterm as shipped | 0.0 | 1 of 10, every repetition | 51.7 |
| with the watchdog | 5.6 | 10 of 10, every repetition | 57.0 |

`src/sync-output.ts` puts a ceiling on how long a repaint can be withheld. It
does not ignore the mode: tearing every frame would be a regression for the
applications the mode exists to serve. If the mode is set and nothing has
painted for `syncOutput.timeoutMs`, it clears the mode, renders synchronously
and hands the mode back. Synchronously is the point; anything scheduled loses
the same race to the next begin-of-update that xterm's own timeout loses.

An application that ends its updates promptly never reaches the ceiling.
A paced application writing each update in two halves 50 ms apart forces zero
repaints and shows only whole frames, in both the fixed and the unfixed build.
Set `syncOutput.timeoutMs` to 0 to disable the watchdog and take xterm's
behaviour unchanged.

Forcing a repaint runs through the code that cancels xterm's 1000 ms timeout, so
the watchdog also takes over what that timeout was for: an application that stops
mid-update and never sends its end. One timeout after the last byte, with the
mode still set, the mode is left clear rather than handed back.

`test/browser/sync-output.spec.mjs` covers all three. The flood test runs a
control arm with the watchdog off that asserts the terminal still draws nothing,
so the fixed arm cannot pass by a fixture that stopped producing the pathology,
and the tearing test runs the same fixture with the ceiling set below the gap to
prove the check can see a torn frame at all.
