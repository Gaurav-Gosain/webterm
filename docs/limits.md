...
Both reaches, and the APC registration alongside them, are confined to [`src/kitty/xterm-adapter.ts`](../src/kitty/xterm-adapter.ts). Nothing else under `src/kitty/` names a private property.
